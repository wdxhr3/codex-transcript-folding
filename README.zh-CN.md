...
- 折叠或展开任意一条已经完成的用户消息；
- 折叠或展开任意一条已经完成的助手回复，并同时收起其关联的工具调用与执行活动；
- 把两条用户消息之间的全部助手输出视为一个回复，只显示一个占位符；
- 折叠当前任务中的全部用户和助手消息；
- 展开当前任务中的全部消息；
...
工具调用不会被自动隐藏。只有用户明确折叠其所属的助手回复时，它们才会随回复一起消失；展开回复后，调用记录会完整恢复。
...
6. 发起一个会产生明显工具调用的任务，例如网页搜索或终端命令。选中对应的助手回复并按 `Space`，预期只出现一条
   `▶ Assistant response collapsed ...` 摘要；关联的工具调用和命令输出应同时消失。
7. 展开同一回复，确认回复文本和关联活动同时恢复。再次折叠后按 `q`，普通 inline 界面应显示相同的占位符。
...
- 工具调用不能单独选择，也不会自动隐藏。它们会归入前后两条用户消息之间的助手回复；折叠该回复或执行“全部折叠”时，关联调用、命令输出和执行活动会一并收起。
